...
class Maze(gym.Env):
	...
	def __init__(	self, 
		...
		# resolucao
		self.res = np.max([np.abs(np.diff(self.xlim)), np.abs(np.diff(self.ylim))])/float(grid_size)
		...
		self.th = np.linspace(0.0, 2.0*np.pi, NACTIONS)[:-1]
	...
	def actionU(self, action):
		...
		# a acao 0 faz ficar parado
		if action == 0:
			return np.zeros(2)
		
		# angulo de movimentacao
		angle = self.th[action-1]
		return self.res*np.array([np.cos(angle), np.sin(angle)])
	...
	def step(self, action):
		...
		# seleciona acao e vai para o proximo estado
		nextp = self.p + self.actionU(action)
		...
		for x in xm:
			for y in ym:
				...
				if np.linalg.norm(u) > 0:
					self.draw_arrow((0, 150, 200), start, end)

		# Desenha trajetoria do robo	
		for i in range(len(self.traj)-1):
			start = self.world_to_screen(self.traj[i])
			end = self.world_to_screen(self.traj[i+1])
			pygame.draw.line(self.screen, (155, 0, 200), start, end, 2)
